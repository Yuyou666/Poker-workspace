...
Card::Card(string rank, string suit) {
    string validSuits[] = {"hearts", "diamonds", "clubs", "spades", "wild"};
    bool valid = false;
    for (const auto& s : validSuits) {
        if (suit == s) { valid = true; break; }
    }
    if (!valid) throw std::invalid_argument("Invalid suit: " + suit);
    this->rank = rank;
    this->suit = suit;
...
Card::Card(string rank, string suit, string cardType, int bonusPoint, int mult) {
    string validSuits[] = {"hearts", "diamonds", "clubs", "spades", "wild"};
    string validTypes[] = {"normal", "bonus", "mult", "wild"};
    bool validSuit = false, validType = false;
    for (const auto& s : validSuits) {
        if (suit == s) { validSuit = true; break; }
    }
    for (const auto& t : validTypes) {
        if (cardType == t) { validType = true; break; }
    }
    if (!validSuit) throw std::invalid_argument("Invalid suit: " + suit);
    if (!validType) throw std::invalid_argument("Invalid card type: " + cardType);
    this->rank = rank;
    this->suit = suit;
...
Card::~Card() {
    std::cout << "[DEBUG] Deleting Card: " << rank << " of " << suit << " (" << cardType << ")" << std::endl;
